...
export default function Services() {
  usePageMeta(
    'Services — Petvet Care | Home vet for cattle, pets & birds, Darbhanga',
    'Cattle & buffalo treatment, dog & cat care, poultry, vaccination, deworming and emergency home visits across Darbhanga and Mithila.'
  );
  ...
  return (
    <div className="services-page">
      <PageHeader
        ...
        tagVariant="tag--leaf"
        mai="हर पशु लेल सेवा"
        title="Doorstep care for every animal"
      ...
      <section className="section">
        <div className="container">
          <div className="svc-list">
            {services.map((s, i) => (
              <Reveal as="article" key={s.id} className={`svc-full svc-full--${s.accent}`} variant="up" delay={(i % 2) * 80}>
                ...
                <div className="svc-full__head">
                  ...
                  <div>
                    ...
                    <span className="mai svc-full__mai">{s.mai}</span>
                  </div>
                </div>
                ...
                <a href={wa(`Namaste Dr. Rajpal, I need help with: ${s.title}. My animal is `)} target="_blank" rel="noreferrer" className="link svc-full__cta">
                  <Icon name="whatsapp" size={16} /> ई बारे मे पुछू
                </a>
              </Reveal>
            ))}
          </div>
      ...
      <section className="section svc-process">
        <div className="container">
          ...
          <div className="svc-process__row">
            {process.map((p) => (
              <Reveal key={p.step} className="svc-process__step" variant="up">
                <span className="svc-process__num mai">{p.step}</span>
                <h3>{p.title}</h3>
                <span className="mai svc-process__mai">{p.mai}</span>
                <p>{p.body}</p>
              </Reveal>
            ))}
          </div>
